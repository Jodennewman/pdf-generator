    ...
    for (const file of files) {
      ...
      if (stat.isDirectory()) {
        ...
        try {
          ...
          // Get module number from config and format it
          const moduleNumber = config.moduleInfo.moduleNumber.toString().replace(/\./g, '-');
          
          // Check if filename already starts with module number
          const prefix = baseFilename.startsWith(moduleNumber) ? '' : `${moduleNumber}-`;
          
          // Generate output paths with module number prefix
          const lightOutputPath = path.join(outputDir, `${prefix}${baseFilename}-light.html`);
          const darkOutputPath = path.join(outputDir, `${prefix}${baseFilename}-dark.html`);
